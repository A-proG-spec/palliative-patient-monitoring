...
import { useMedicationQueue } from '@/hooks/useMedicationQueue';

const MedicationOrdersPage: React.FC = () => {
  const navigate = useNavigate();

  const { data, isLoading, isError, error, refetch } = useMedicationQueue();
            ...
            <tbody className="divide-y divide-border-base">
              {pendingOrders.map((order) => (
                <tr
                  key={order.id}
                  onClick={() =>
                    navigate(ROUTES.MEDICATION_ORDER_DETAIL(String(order.id)))
                  }
